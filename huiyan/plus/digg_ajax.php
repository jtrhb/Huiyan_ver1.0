...
if(!is_array($row) || $cfg_digg_update==0)
{
  ...
  $row2 = $dsql->GetOne("SELECT count(id) c FROM  `dede_member_stow` WHERE aid='$id'");  
...
else
{
    ...
    $digg = '<div class="like-box">
            <i class="pos-btn" title="喜欢" onclick="javascript:postDigg(\'good\','.$id.')">'.$row['goodpost'].'</i>
        </div>
        <div class="favorite-box">
            <i class="pos-btn" title="收藏" onClick="window.open(\'/plus/stow.php?aid='.$id.'\')">'.$row2['c'].'</i>
        </div>
        <div class="nolike-box">
            <i class="pos-btn" title="没劲" onclick="javascript:postDigg(\'bad\','.$id.')">'.$row['badpost'].'</i>
        </div>';
}
AjaxHead();
echo $digg;
