...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == '请输入搜索内�?') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = '请输入搜索内�?';
	} else {
...
function cardInit() {
	var cardShow = function (obj) {
		...
		pos = obj.getAttribute('c') == '2' ? '43' : (obj.getAttribute('c') == '1' ? '43' : obj.getAttribute('c'));
		if(obj.getAttribute('c') == '2'){
			USERCARDST = setTimeout(function() {ajaxmenu(obj, 100, 1, 2, pos, null, 'p_pop card2');}, 250);
		}else{
