...
else if($action=='comment')
{
    ...
    if(empty($channelid))
    {
        ...
        $query = "SELECT arc.id,arc.typeid,arc.arcrank,arc.channel,arc.pubdate,arc.senddate,arc.ismake,f.arctitle,f.aid,f.dtime,f.msg
                 ...
                 WHERE f.mid='{$_vars['mid']}' $addqSql ORDER BY arc.id DESC";
    }
    else
    {
        $channelid = intval($channelid);
        ...
        if($chRow['issystem']==-1)
        {
            ...
            $query = "SELECT arc.id,arc.typeid,arc.arcrank,arc.channel,arc.pubdate,arc.senddate,arc.ismake,f.arctitle,f.aid,f.dtime,f.msg
                 ...
                 WHERE f.mid='{$_vars['mid']}' And arc.channel='$channelid' $addqSql ORDER BY arc.aid DESC";
        }
        else
        {
            $query = "SELECT arc.id,arc.typeid,arc.arcrank,arc.channel,arc.pubdate,arc.senddate,arc.ismake,f.arctitle,f.aid,f.dtime,f.msg
                 ...
                 WHERE f.mid='{$_vars['mid']}' And arc.channel='$channelid' $addqSql order by arc.id desc";
        }
    }
    $dlist = new MemberListview();
    $dlist->pageSize = $_vars['pagesize'];
    ...
    $dlist->SetParameter("channelid", $channelid);
    $dlist->SetParameter("action", $action);
    $dlist->SetTemplate(DEDEMEMBER."/space/{$_vars['spacestyle']}/listcomment.htm");
    $dlist->SetSource($query);
    $dlist->Display();
    exit();
...
else if($action=='guestbook')
{
    ...
    left join `#@__member` mb on mb.userid=mg.uname 
